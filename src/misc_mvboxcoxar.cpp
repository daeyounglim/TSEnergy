				    ...
				    ) 
{
	...
	double loglik = 2.0 * zij - 2.0 * std::log1p(std::exp(2.0 * zij)); // Jacobian for Fisher's z-transformation
	for (int t = 1; t < T; ++t) {
				    ...
				    )
{
	...
	arma::mat w(arma::size(ystar), arma::fill::zeros); // K * T matrix
    for (int t = 0; t < T; ++t) {
        for (int k = 0; k < K; ++k) {
            if (lam(k,t) == 0.0) {
	...
	double loglik = -0.5 * arma::dot(xx, Siginv * xx) + arma::dot(lam - 1.0, arma::log(yobs - a))
					+ R::dnorm4(cij, 0.0, std::sqrt(sigc2), 1);
	for (int t = 1; t < T; ++t) {
		arma::vec xxx = resid.col(t) - A * w.col(t-1);
		loglik -= 0.5 * arma::dot(xxx, Siginv * xxx);
	}
...
{
	const int K = resid_prop.n_rows;
	const int T = resid_prop.n_cols;

	double loglik = arma::dot(lam_prop - 1.0, arma::log(yobs - a)) + R::dnorm4(cij_prop, 0.0, std::sqrt(sigc2), 1);
	for (int t = 0; t < T; ++t) {
		arma::vec xxx = resid_prop.col(t);
		loglik -= 0.5 * arma::dot(xxx, Siginv * xxx);
	}
	return loglik;
}
...
double loglik_C_amh(const arma::mat& resid,
	...
	double loglik = log_jacobian;
	for (int t = 0; t < T; ++t) {
		arma::vec xxx = resid.col(t);
		loglik -= 0.5 * arma::dot(xxx, Siginv * xxx);
	}
	return loglik;
}
